...
func (s *dynamicSubScheduler) runNode(child Node, input any, opts runNodeOptions) (any, error) {
	...
	// A child whose WaitForOutput is set but that finished without an
	// output is not done yet; pause so the parent re-runs and re-invokes
	// RunNode. WaitForOutput mirrors adk-python's wait_for_output node
	// field. This achieves the same effect as adk-python calling
	// run_node(raise_on_wait=True), but unconditionally rather than via a
	// per-call flag — adk-go has no raise_on_wait equivalent yet (safe
	// today: dispatchTaskFC is the only RunNode caller; see
	// followups/raise-on-wait-parity.md).
