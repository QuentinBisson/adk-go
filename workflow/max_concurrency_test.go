...
// fans out from START and feeds a single JoinNode terminal. This keeps
// the graph single-terminal-output (the join) so the scheduler's
// finalize check is satisfied while the fanned-out nodes still run
// concurrently. Returns the edges and the join node.
