...
func (n *contentOutputNode) Run(ctx agent.InvocationContext, _ any) iter.Seq2[*session.Event, error] {
...
func (n *eventPassthroughNode) Run(ctx agent.InvocationContext, _ any) iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		ev := session.NewEvent(ctx.InvocationID())
		ev.Output = n.inner
		yield(ev, nil)
...
func TestIntegration_FunctionNode_StructOutput(t *testing.T) {
	...
	node := NewFunctionNode("make_ticket",
		func(ctx agent.InvocationContext, _ any) (ticket, error) {
			return want, nil
		}, defaultNodeConfig)
...
func TestIntegration_FunctionNode_OutputSchemaMismatch(t *testing.T) {
	...
	node, err := NewFunctionNodeWithSchema[any, map[string]any]("bad",
		func(ctx agent.InvocationContext, _ any) (map[string]any, error) {
...
func TestIntegration_EndToEndChain(t *testing.T) {
	...
	seed := NewFunctionNode("seed",
		func(ctx agent.InvocationContext, _ any) (rawOrder, error) {
			return rawOrder{Item: "widget", Qty: 3}, nil
		}, defaultNodeConfig)
	price := NewFunctionNode("price",
		func(ctx agent.InvocationContext, in rawOrder) (pricedOrder, error) {
...
func TestIntegration_TerminalOutputConstraint(t *testing.T) {
	...
	t.Run("multiple_terminals_error", func(t *testing.T) {
		a := NewFunctionNode("a",
			func(ctx agent.InvocationContext, _ any) (string, error) { return "A", nil },
			defaultNodeConfig)
		b := NewFunctionNode("b",
			func(ctx agent.InvocationContext, _ any) (string, error) { return "B", nil },
			defaultNodeConfig)
...
func TestIntegration_ToolNode_ResultUnwrap(t *testing.T) {
	...
	seed := NewFunctionNode("seed",
		func(ctx agent.InvocationContext, _ any) (Input, error) {
