...
func TestScheduler_Finalize_SingleTerminalOutput(t *testing.T) {
	...
	t.Run("named workflow surfaces its name in the error", func(t *testing.T) {
		// Mirrors adk-python's Workflow._finalize message, which
		// prefixes the error with the workflow name so callers
		// can tell which workflow failed in a nested setup.
		mockCtx := newMockCtx(t)
		a := outputFnNode("a", "A")
		b := outputFnNode("b", "B")
		w, err := New("billing_pipeline", []Edge{
			{From: Start, To: a},
			{From: Start, To: b},
		})
		if err != nil {
		...
		if !errors.Is(gotErr, ErrMultipleTerminalOutputs) {
			t.Fatalf("got error %v, want ErrMultipleTerminalOutputs", gotErr)
		}
		if !strings.Contains(gotErr.Error(), `"billing_pipeline"`) {
			t.Errorf("error %q does not include workflow name", gotErr.Error())
		}
	})
}
